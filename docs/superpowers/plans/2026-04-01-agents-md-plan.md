...
**Goal:** Create `AGENTS.md` in the agents4geosx repo root that formalizes the system architecture: taxonomy, capability tiers, agent registry, tool inventory, knowledge modules, coordination patterns, and extension guide.

**Architecture:** Single markdown file with 7 sections. No code changes — purely documentation. Cross-referenced against source code for accuracy, with a pointer added to CLAUDE.md.
...
|--------|------|---------------|
| Create | `AGENTS.md` | Full architecture document (7 sections) |
| Modify | `CLAUDE.md` | Add pointer to AGENTS.md in Architecture section |

---

### Task 1: Write AGENTS.md with taxonomy, tiers, and agent registry

**Goal:** Create the core AGENTS.md file with Sections 1-3.

**Files:**
- Create: `AGENTS.md`

**Acceptance Criteria:**
- [ ] Taxonomy defines all 5 entity types (Agent, Tool, Knowledge module, State, Hook)
- [ ] Capability tiers table with Tier 1/2/3 mapped to Haiku/Sonnet/Opus
- [ ] All 11 agents listed with tier, description, tools, knowledge deps, inputs, outputs
- [ ] No placeholders or TBDs

**Verify:** `head -200 AGENTS.md` — confirms structure and content

**Steps:**

- [ ] **Step 1: Create AGENTS.md with header and Section 1 (Taxonomy)**
...
- [ ] **Step 2: Write Section 2 (Capability Tiers & Model Routing)**
...
- [ ] **Step 3: Write Section 3 (Agent Registry)**
...
Run: `head -200 AGENTS.md`
Expected: Document header, taxonomy table, tiers table, first several agent entries visible.

- [ ] **Step 5: Commit**

```bash
git add AGENTS.md
git commit -m "docs: add AGENTS.md with taxonomy, tiers, and agent registry (sections 1-3)"
```

---

### Task 2: Add tool inventory, knowledge modules, coordination patterns, and extension guide

**Goal:** Complete AGENTS.md with Sections 4-7.

**Files:**
- Modify: `AGENTS.md`

**Acceptance Criteria:**
- [ ] Tool inventory table covers all 46+1 tools with group, name, purpose, used-by columns
- [ ] Knowledge modules table with provenance and consumed-by columns
- [ ] Three coordination patterns documented (Pipeline, Fan-out, Feedback loop)
- [ ] Extension guide has conventions, template, and checklist
- [ ] No placeholders or TBDs

**Verify:** `wc -l AGENTS.md` — expect ~350-450 lines

**Steps:**

- [ ] **Step 1: Write Section 4 (Tool Inventory)**
...
- [ ] **Step 2: Write Section 5 (Knowledge Modules)**
...
- [ ] **Step 3: Write Section 6 (Coordination Patterns)**
...
- [ ] **Step 4: Write Section 7 (Extension Guide)**
...
```bash
git add AGENTS.md
git commit -m "docs: complete AGENTS.md with tool inventory, knowledge, patterns, extension guide (sections 4-7)"
```

---

### Task 3: Cross-reference AGENTS.md against source code and update CLAUDE.md

**Goal:** Verify AGENTS.md accuracy against actual source code and add a pointer from CLAUDE.md.

**Files:**
- Modify: `AGENTS.md` (fix any discrepancies found)
- Modify: `CLAUDE.md` (add reference to AGENTS.md)

**Acceptance Criteria:**
- [ ] Every tool listed in AGENTS.md exists as an `@mcp.tool` in source
- [ ] Every skill file listed in AGENTS.md exists in `skills/`
- [ ] Every knowledge module listed exists in `knowledge/`
- [ ] CLAUDE.md references AGENTS.md in its Architecture section
- [ ] Final commit with both files

**Verify:** `grep -c '@mcp.tool' src/agents4geosx/tools/*.py` — tool count matches inventory

**Steps:**

- [ ] **Step 1: Count tools in source**

```bash
grep -c '@mcp.tool' src/agents4geosx/tools/*.py
```

Expected output (approximate):
```
src/agents4geosx/tools/fluid_tools.py:10
src/agents4geosx/tools/mesh_tools.py:8
src/agents4geosx/tools/postproc_tools.py:7
src/agents4geosx/tools/schema_tools.py:7
src/agents4geosx/tools/xml_tools.py:14
```

Total: 46 (+ `health_check` in server.py + `sanity_check`). Compare against AGENTS.md inventory.
...
Expected: 11 files matching the 11 agents in the registry. Compare names.

- [ ] **Step 3: List knowledge modules**

```bash
ls src/agents4geosx/knowledge/ knowledge/
```

Expected: 4 Python files in `src/.../knowledge/` + `lessons_learned.md` in `knowledge/`.

- [ ] **Step 4: Fix any discrepancies in AGENTS.md**

If tool counts don't match, or skill files are named differently, update AGENTS.md.

- [ ] **Step 5: Add AGENTS.md reference to CLAUDE.md**

In CLAUDE.md, update the Architecture section to include:
...
git commit -m "docs: cross-reference AGENTS.md against source, add pointer from CLAUDE.md"
```
